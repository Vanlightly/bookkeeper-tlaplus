--------------------------- MODULE MessagePassingUnboundedLedgers ---------------------------
...
ReadTimeoutForBookie(msgs, cid, bookie, term) ==
...
WriteTimeoutForBookie(msgs, cid, bookie, term) ==
    \E msg \in DOMAIN msgs :
        /\ msgs[msg] = -1
        /\ msg.bookie = bookie
        /\ msg.cid = cid
...
ReadTimeoutCount(cid, ensemble, term) ==
    IF \E b \in ensemble : ReadTimeoutForBookie(messages, cid, b, term)
    THEN Cardinality({ b \in ensemble : ReadTimeoutForBookie(messages, cid, b, term)})
    ELSE 0

ClearWriteTimeout(cid, bookies, term) ==
...
\* never get delivered
IgnoreFurtherReadResponses(msg, ensemble, term) ==
...
\* Last modified Mon Dec 06 12:49:52 CET 2021 by GUNMETAL
