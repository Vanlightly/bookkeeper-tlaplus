...
1. Built on the changes of the BookKeeperProtocolGuaranteedWriteQuorum spec. This Unbounded
   ...
   of recovery writes (normal writes can use existing ack quorum guarantee).
   A separate design using terms is required when using the existing behaviour where only
   ack quorum is guaranteed at all times. If you use this design with existing AQ behaviour, there is the
...
3. Fencing is now based on integer terms rather than a boolean value. When a client decides it should 
...
4. Recovery is performed now at the beginning of a term, rather than just when closing a ledger.
...
5. After discovering the last recoverable entry (or there were none) a final empty entry
   is written back with the "truncate" flag. Bookies that receive an add request with the
   truncate flag delete any ledger entries with a higher entry id.
*)
...
CONSTANTS Bookies,                      \* The bookies available e.g. { B1, B2, B3, B4 }
          ...
          Values,                       \* The data items to send. Limited to a very small number of data items
                                        \* in order to keep the state space small. E.g {v1} or {v1, v2}
...
\* auxilliary variables, add these to a VIEW to reduce state space
VARIABLES sent_values,
          acked_values
...
vars == << bookie_vars, clients, meta_vars, messages, sent_values, acked_values >>
...
\* The maximum number of entries corresponds to the values that can be sent
\* and the number of term changes which is 1 less than the number of clients
EntryIds ==
    1..Cardinality(Values)+Cardinality(Clients)-1

NilEntry == [id |-> 0, data |-> Nil]

Entry ==
    [id: EntryIds, data: Values \union {Nil, NoOp}]
...
ClientState ==
    [id: Clients,
     ...
     trunc_entry: Nat \union {Nil}]             \* The empty truncate entry at the end of recovery
...
ClientCreatesLedger(cid) ==
    ...
    /\ UNCHANGED << bookie_vars, meta_last_entry, messages, sent_values, acked_values >>
...
- the trunc flag is included here as true/false but an implementation
...
ClientSendsAddEntryRequests(cid) ==
    ...
    IN
        ...
        /\ \E v \in Values : v \notin sent_values
        /\ LET entry_data == CHOOSE v \in Values : v \notin sent_values
           IN
            /\ SendAddEntryRequests(c, [id   |-> c.lap + 1, data |-> entry_data])
            /\ sent_values' = sent_values \union {entry_data}
        /\ UNCHANGED << bookie_vars, meta_vars, acked_values >>
...
\* if it already exists, then overwrite, else append
AddEntry(b, entry) ==
    IF \E e \in b_entries[b] : e.id = entry.id
    THEN
        { IF e.id = entry.id THEN entry ELSE e : e \in b_entries[b] }
    ELSE
        b_entries[b] \union {entry}

AddEntryAndTruncate(b, entry) ==
    { e \in b_entries[b] : e.id < entry.id } \union {entry}
...
BookieSendsAddConfirmedResponse ==
    \E msg \in DOMAIN messages :
        ...
        /\ b_entries' = [b_entries EXCEPT ![msg.bookie] = 
                                IF msg.trunc 
                                THEN AddEntryAndTruncate(msg.bookie, msg.entry)
                                ELSE AddEntry(msg.bookie, msg.entry)]
        ...
        /\ UNCHANGED << clients, meta_vars, sent_values, acked_values >>
...
MaxContiguousEntryAtQuorum(c, confirmed, quorum) ==
    IF c.lac < c.lap
    THEN 
        \* there is an entry that has reached quorum and whose prefix has reached quorum
        IF \E id \in (c.lac+1)..c.lap : \A id1 \in (c.lac+1)..id :
                                            Cardinality(confirmed[id1]) >= quorum
        THEN 
            \* choose the entry that has reached quorum and whose prefix has reached quorum
            CHOOSE id \in (c.lac+1)..c.lap : 
                /\ \A id1 \in (c.lac+1)..id : Cardinality(confirmed[id1]) >= quorum
                \* such that there also isn't another one that also is contiguous but higher
                /\ ~\E other_id \in (c.lac+1)..c.lap :
                    /\ other_id > id 
                    /\ \A other_id1 \in (c.lac+1)..other_id : 
                        Cardinality(confirmed[other_id1]) >= quorum
        ELSE c.lac
    ELSE c.lac
...
MayBeAck(c, msg, lac) ==
    ...
    THEN 
        acked_values' = IF lac >= msg.entry.id
                        THEN acked_values \union { msg.entry.data }
                        ELSE acked_values
    ELSE UNCHANGED acked_values                            
...
ReceiveAddConfirmedResponse(c) ==
    \E msg \in DOMAIN messages :
        ...
        /\ LET confirmed == AddToConfirmed(c, msg.entry.id, msg.bookie)
           IN LET lac  == MaxContiguousEntryAtQuorum(c, confirmed, AckQuorum)
                  lafr == MaxContiguousEntryAtQuorum(c, confirmed, WriteQuorum)
              IN
                /\ clients' = [clients EXCEPT ![c.id] = 
...
ClientReceivesAddConfirmedResponse(cid) ==
    ...
    /\ UNCHANGED << bookie_vars, meta_vars, sent_values >>
...
ClientChangesEnsemble(cid) ==
    ...
    /\ UNCHANGED <<  bookie_vars, meta_status, meta_term, meta_last_entry, sent_values, acked_values >>
...
ClientClosesLedgerSuccess(cid) ==
    ...
    IN
        ...
        /\ meta_version' = meta_version + 1
        /\ UNCHANGED << bookie_vars, meta_term, meta_fragments, messages, sent_values, acked_values >>
...
ClientClosesLedgerFail(cid) ==
    ...
    IN 
        ...
        /\ clients' = [clients EXCEPT ![cid] = [@ EXCEPT !.status = CLIENT_WITHDRAWN,
                                                         !.meta_version = Nil]]
        /\ UNCHANGED << bookie_vars, meta_vars, messages, sent_values, acked_values >>
...
ClientStartsNewTerm(cid) ==
    ...
    /\ UNCHANGED << bookie_vars, meta_fragments, meta_last_entry, sent_values, acked_values >>
...
BookieSendsFencingReadLafrResponse ==
    \E msg \in DOMAIN messages :
        ...
        /\ UNCHANGED << b_entries, b_lac, b_lafr, clients, meta_vars, sent_values, acked_values >>
...
the final truncate entry is appended to the pending add ops,
...
NewAddOp(id, data, fragment) ==
    [entry       |-> [id   |-> id,
                      data |-> data],
     fragment_id |-> fragment.id,
     ensemble    |-> fragment.ensemble]
...
ReadPositive(c, msg, responses) ==
    /\ clients' = [clients EXCEPT ![c.id] = 
                        [c EXCEPT !.curr_read_entry = Nil, \* reset for next read
                                  ...
                                  !.lap             = @ + 1,
                                  !.pending_add_ops = @ \union {NewAddOp(c.lap + 1, 
                                                                         msg.entry.data, 
                                                                         c.curr_fragment)}]]
...
ReadNegative(c, msg, responses) ==
    /\ clients' = [clients EXCEPT ![c.id] = 
                    [c EXCEPT !.curr_read_entry        = Nil, \* reset to reduce state space
                              ...
                              !.lap                    = @ + 1,
                              !.trunc_entry            = msg.entry.id,
                              !.pending_add_ops        = @ \union {NewAddOp(msg.entry.id,
                                                                            NoOp,
                                                                            c.curr_fragment)}]]
    /\ IgnoreFurtherReadResponses(msg, c.recovery_ensemble, c.term)
...
- The term must be included to prevent clients of previous terms from also
  attempting recovery.
- If this entry is the last truncate entry, then set the trunc flag.
****************************************************************************)
    ...
    IN
        ...
        /\ \E op \in c.pending_add_ops :
            ...
            /\ ~\E op2 \in c.pending_add_ops :
                /\ NotSentWrite(c, op2)
                /\ op2.entry.id < op.entry.id
            /\ LET trunc_flag == (c.trunc_entry # Nil
                                  /\ c.trunc_entry = op.entry.id)
...
- The client is only able to change the ledger to OPEN if the metadata store 
  ...
  aborted.
- Recovery is only complete once all pending add ops are completed (meaning 
  they have reached WQ) and therefore the truncate signal has been processed
  by every bookie in the last fragment. 
****************************************************************************)
...
ClientChangesTermToOpen(cid) ==
    ...
    IN
        ...
        /\ meta_status' = STATUS_OPEN
        /\ UNCHANGED << bookie_vars, messages, meta_term, meta_last_entry, sent_values, acked_values >>
...
This divergence is known as Ledger Truncation and is data loss.                                       
****************************************************************************)
...
(***************************************************************************
Invariant: All committed values are readable                           

This invariant is violated if, once an entry value has been acknowledged, it  
is no longer readable from its ensemble. This differs from the id based
readability as ids can be reused between terms, but values cannot in this spec.
****************************************************************************)

AllAckedValuesAreReadable ==
    \A v \in acked_values :
        \E entry_id \in EntryIds :
            \E b \in FragmentOfEntryId(entry_id, meta_fragments).ensemble : 
                \E entry \in b_entries[b] :
                    entry.data = v
...
(***************************************************************************
Invariant: No log divergence                                                        

Any given committed entry has the same value on all bookies, as long as:
- the bookie is in the entry writeset
- the entry is at or below the LAC on that bookie.

Any entries not in the writeset or higher than the LAC are
dirty entries and are not readable by reader clients. 
***************************************************************************)
    
BookieHasCommittedEntry(b, entry_id) ==
    /\ b_lac[b] >= entry_id
    /\ \E entry \in b_entries[b] :
        entry.id = entry_id
            
NoLogDivergence ==
    IF \E cid \in Clients : clients[cid].lac > 0
    THEN
        LET max_cid   == CHOOSE cid1 \in Clients :
                            \A cid2 \in Clients : clients[cid1].lac >= clients[cid2].lac
            max_entry == clients[max_cid].lac
        IN             
            \A entry_id \in 1..max_entry :
                LET fragment == FragmentOfEntryId(entry_id, meta_fragments)
                IN
                    \A b1, b2 \in Bookies :
                        IF b1 \in fragment.ensemble
                            /\ b2 \in fragment.ensemble
                            /\ BookieHasCommittedEntry(b1, entry_id) 
                            /\ BookieHasCommittedEntry(b2, entry_id)
                        THEN LET entry1 == CHOOSE entry \in b_entries[b1] : entry.id = entry_id
                                 entry2 == CHOOSE entry \in b_entries[b2] : entry.id = entry_id
                             IN entry1.data = entry2.data
                        ELSE TRUE
    ELSE TRUE
