...
(*
Models the use of the LIMBO ledger status to avoid data loss when a bookie loses data.
See the LIMBO_README.md for more information.
*)
...
CONSTANTS Bookies,                      \* The bookies available e.g. { B1, B2, B3, B4 }
          ...
          AllowCrashDuringReplication,  \* Allows a crash with data loss to occur during replication
          AllowCrashDuringFencing,      \* Allows a crash with data loss to occur during the fencing phase of recovery
          AllowCrashDuringRecoveryReads,\* Allows a crash with data loss to occur during the recovery read phase of recovery
          UseLimboStatus                \* Use ledger limbo status to avoid arbitrary failures when a bookie restarts with data loss
...
ASSUME AllowCrashDuringReplication \in BOOLEAN
ASSUME AllowCrashDuringFencing \in BOOLEAN
ASSUME AllowCrashDuringRecoveryReads \in BOOLEAN
ASSUME UseLimboStatus \in BOOLEAN
...
W2TriesInvalidEnsembleChange ==
    ...
    /\ UNCHANGED <<  bookie_vars, w1, meta_vars, messages, crashes >>
...
BookieRestartsEmptyDuringReplication ==
    /\ AllowCrashDuringReplication
    /\ crashes = 0
    /\ meta_status = STATUS_OPEN
    /\ BookieRestartsEmpty

BookieRestartsEmptyDuringRecoveryFencing ==
    /\ AllowCrashDuringFencing
    /\ crashes = 0
    /\ meta_status = STATUS_IN_RECOVERY
    /\ w2.lac = Nil
    /\ BookieRestartsEmpty

BookieRestartsDuringRecoveryReadPhase ==
    /\ AllowCrashDuringRecoveryReads
    /\ crashes = 0
    /\ meta_status = STATUS_IN_RECOVERY
    /\ w2.lac # Nil
    /\ BookieRestartsEmpty    
...
\* Last modified Sat Feb 27 13:35:34 CET 2021 by jvanlightly
\* Last modified Sat Feb 27 13:28:07 CET 2021 by GUNMETAL
\* Last modified Sat Feb 27 13:04:33 CET 2021 by GUNMETAL
